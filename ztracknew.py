...
db_path = os.path.join(BASE_DIR, "trax.db")
...
def download_log():
    file_path = os.path.abspath("log_produzione.xlsx")
...
async function login(){
 try{
   ...
   let r = await fetch('http://127.0.0.1:8010/login',{
  method:'POST',
...
def salva_dati(data):
    ...
    BASE_DIR = os.path.dirname(os.path.abspath(__file__))
    file = os.path.join(BASE_DIR, "log_produzione.xlsx")

    new_row = {
        "data": datetime.now().strftime("%d/%m/%Y %H:%M"),
        "nome_kit": data.name,
        "ospedale": data.hospital,
...
if __name__ == "__main__":
    print("🚀 Avvio applicazione completa...")
